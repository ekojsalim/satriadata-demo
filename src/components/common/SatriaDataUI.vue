    ...
    <v-container fluid>
      ...
      <v-layout row wrap justify-space-around class="image-panel elevation-1">
        ...
        <v-flex sm6 md4 column fill-height class="output-container">
          ...
          <v-flex class="inference-time-class">
            <span class="inference-time">Age: </span>
            <span v-if="outputAge > 0" class="inference-time-value"
              >{{ outputAge.toFixed(1) }}
            </span>
            <span v-else>-</span>
          </v-flex>
...
export default class SatriaDataUI extends Vue {
  ...
  constructor() {
    ...
    this.backendSelectList = [
      { text: "GPU-WebGL (Not Available)", value: "webgl" },
  ...
  get outputClasses() {
    return this.getPredictedClass(Array.prototype.slice.call(this.output?.[1] ?? []));
  }

  get outputAge() {
    return this.getAge(Array.prototype.slice.call(this.output?.[0] ?? []));
  }
  ...
  async runModel() {
    ...
    this.output = tensorOutput.map((v) => v.data);
    this.sessionRunning = false;
  }
